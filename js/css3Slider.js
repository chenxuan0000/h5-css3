...
(function($) {
    $.fn.css3Slider = function(options) {
        var opts = $.extend({}, $.fn.css3Slider.defaults, options),
            ...
            keyframes = "@keyframes scroll{0% {transform: translateZ( -" + translateZ + "rem) rotateY(0deg);}\n" +
            "100%{transform: translateZ( -" + translateZ + "rem) rotateY(360deg);}}";
        // 设置场景元素(父元素的基本属性)
        $father.css({
            'width': opts.containerWidth + 'rem',
            'height': opts.containerHeight + 'rem',
            'perspective': opts.perspective + 'rem',
            'perspectiveOrigin': opts.rotateX + ' ' + opts.rotateY
        });
        //子元素初始z偏移
        $children.css("transform", " translateZ( -" + translateZ + "rem)");
        //遍历设置孙子元素的rotateY
        $grandson.each(function(i, e) {
            e.style.transform = 'rotateY( ' + i * deg + 'deg) translateZ( ' + translateZ + 'rem)';
        });
        // 动态插入keyframes
        ...
        var setChildCss3 = function(delay, y) {
            $children.css({
                ...
                "transform": " translateZ( -" + translateZ + "rem) rotateY(" + y + "deg)"
            });
        };
        // mousewheel 鼠标滚动事件
